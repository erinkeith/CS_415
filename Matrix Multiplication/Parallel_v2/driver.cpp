...
void master()
{
 ...
 for( i=1, row=0; i<numProcs; i++, row++ )
 {
  ...
  MPI_Send( &matA[row][0], MAX, MPI_INT, i, DATA_TAG, MPI_COMM_WORLD );
 }

 // loop to receive matC
 while( numRows<d )
 {
  MPI_Recv( &rcvd, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &stat );
  MPI_Recv( &matC[rcvd][0], MAX, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &stat );
  ...
  MPI_Send( &matA[row][0], MAX, MPI_INT, procID, DATA_TAG, MPI_COMM_WORLD );
  row++;
 }
 ...
 for( i=1; i<numProcs; i++ )
 {
  ...
  MPI_Send( &matA[0][0], MAX, MPI_INT, i, DEL_TAG, MPI_COMM_WORLD );
 }
